...
contract Auction {
    ...
    bool flagRefund = false;
    bool flagFinalize = false;
    mapping (address => uint) refundTable;
    mapping (address => bytes32) bidTable;
    ...
    function withdraw() public {
        //TODO: place your code here
        if (flagFinalize) {
            sellerAddress.transfer(winningPrice);
            flagFinalize = false;
        }
        else if (flagRefund) {
            winnerAddress.transfer(winningPrice);
            flagRefund = false;
        }

        if (refundTable[msg.sender] != 0) {
            msg.sender.transfer(refundTable[msg.sender]);
            refundTable[msg.sender] = 0;
        }
    }
